...
assert.match(sharedSiteJs,/document\.addEventListener\("DOMContentLoaded",(?:initNetEngineerLabSite|start)/);
assert.doesNotMatch(sharedSiteJs,/window\.addEventListener\("load",init/);
assert.match( sharedSiteJs, /<h[23]>\$\{copy\.name\|\|tool\.id\}<\/h[23]>/);
assert.match(
 sharedSiteJs,
 /document\.querySelectorAll\("\[data-tool-grid\]"\)/
);
...
assert.match(toolsCss,/--muted:(#53677f|#65768e)/);
assert.match(
  toolsCss,
  /\.filter\.active \.filter-count\{background:(var\(--primary-dark\)|#ffffff2b)/
);
