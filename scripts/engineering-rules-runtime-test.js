...
assert.throws(()=>evaluateRules([rules[0]],facts,{customOperators:{equals:{id:"equals",version:"1.0.0",handler:()=>true}}}),/Cannot override/);
assert.throws(()=>evaluateRules([{...rules[0],condition:{operator:"custom",params:{}}}],facts,{customOperators:{custom:{id:"custom",version:"1.0.0",handler:()=>"yes"}}}),/invalid outcome/);
assert.throws(()=>evaluateRules([{...rules[0],condition:{operator:"custom",params:{}}}],facts,{customOperators:{custom:()=>true}}),/Invalid custom operator descriptor/);
...
const customDescriptor={id:"acl-shadowed-by-prior-rule",version:"1.0.0",handler:()=>({matched:true,rootCauseKey:"acl-shadow",evidence:{sourceLine:20,priorLine:10}})};
const customFindings=evaluateRules([customRule],facts,{customOperators:{"acl-shadowed-by-prior-rule":customDescriptor}});
...
try{
  ...
  assert.deepStrictEqual(fs.readdirSync(outputRoot).sort(),["keep.txt",first.fileName].sort());assert.strictEqual(first.bundle.rules.length,10);assert(!first.content.includes("website/data/engineering-rules"));
  ...
  const aclPolicyDescriptor={id:"acl-policy-check",version:"1.0.0",handler:()=>false};
  validateBundleCompatibility(loaded);assert.deepStrictEqual(evaluateBundle(loaded,{rules:[]},{customOperators:{"acl-policy-check":aclPolicyDescriptor}}),[]);
  assert.throws(()=>evaluateBundle(loaded,facts,{customOperators:{"acl-shadowed-by-prior-rule":{...customDescriptor,version:"9.9.9"}}}),/version mismatch/);
