...
const activeTools=tools.filter(x=>x.status==="active");
assert(activeTools.length>0);
...
const flowIds=flow.steps.slice().sort((a,b)=>a.order-b.order).map(x=>x.toolId);
assert(flowIds.includes("network-rack-power-cooling-calculator"));
assert(flowIds.includes("48v-battery-runtime"));
assert(flowIds.includes("telecom-solar-battery-sizing-calculator"));
assert(flowIds.includes("pue-data-center-energy-efficiency"));
assert(flowIds.indexOf("network-rack-power-cooling-calculator")<flowIds.indexOf("telecom-solar-battery-sizing-calculator"));
assert(flowIds.indexOf("telecom-solar-battery-sizing-calculator")<flowIds.indexOf("pue-data-center-energy-efficiency"));
...
const prod=JSON.parse(fs.readFileSync(path.join(root,"docs/PRODUCTION_ACCEPTANCE_REPORT.json"),"utf8"));assert.equal(prod.status,"PASS");assert.equal(prod.activeTools,activeTools.length);assert.equal(prod.errors.length,0);assert.equal(prod.warnings.length,0);
