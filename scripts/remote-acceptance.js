...
(async()=>{
  ...
  record("sitemap URL count",urls.length===40,String(urls.length));
  ...
  const report={version:"1.8.3",base,generatedAt:new Date().toISOString(),errors:[error.stack||error.message||String(error)],warnings,status:"FAIL"};
  fs.writeFileSync(path.join(docs,"REMOTE_ACCEPTANCE_REPORT.json"),JSON.stringify(report,null,2)+"\n");
