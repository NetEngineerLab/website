...
线上Service Worker不是当前构建文件，或响应中既没有`no-cache`、`no-store`、`max-age=0`缓存指令，也没有格式合法的`ETag`。Cloudflare Pages始终可以使用`ETag`和`If-None-Match`完成浏览器缓存重验证，因此不要求每个节点必须同时返回`Cache-Control: max-age=0`。
