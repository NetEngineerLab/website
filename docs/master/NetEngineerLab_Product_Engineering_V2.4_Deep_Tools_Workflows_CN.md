...
## 41. 页面级实施标准：以 Fabric 容量规划器为基准

以后新增或深度改造工具页面，必须达到本节标准。数据中心 Fabric 容量规划器是可复用的页面基准，页面验收以“用户能完成一次工程判断并带走成果”为目标。

### 41.1 页面结构与信息层级

- 保留统一的 NetEngineerLab 页面外壳、导航、语言切换和工具标题区。
- 首屏先说明工具解决的问题，再呈现输入区；结果区必须紧邻输入区，避免用户在页面中来回寻找。
- 结果区同时显示关键指标、利用率、瓶颈、状态和建议，状态文字必须能让用户直接判断是否需要调整。
- 移动端使用单列布局，卡片、按钮和表格不能溢出或错位；中英文页面的结构和功能必须一致。

### 41.2 计算与默认场景

- 默认参数必须形成可解释、可继续规划的健康基线，不得首次打开就给出容量失败，除非该工具的真实业务就是风险告警。
- 所有状态必须由同一计算引擎产生；页面文案、方案比较和导出文件不得自行重复计算。
- 至少提供当前方案、均衡调整方案和扩容方案三种可比较场景，并明确显示正常态、N-1/故障态及增长后结果。
- 出现失败或超过目标阈值时，必须同时给出导致失败的瓶颈和可执行的调整方向。

### 41.3 工程成果输出

- 深度工具至少提供打印/工程报告和 BOM（或等价交付清单）两种输出。
- JSON、CSV、打印内容必须包含工具名称、生成时间、语言和完整网站地址；中文页面使用 `https://netengineerlab.com/tools/<slug>/zh/`，英文页面使用 `https://netengineerlab.com/tools/<slug>/`。
- 导出操作必须有可见的成功或失败反馈，文件名应能识别工具和成果类型。
- 页面上应显示可点击的网站地址，打印时地址仍应出现在工程成果中，便于转发后追溯来源。

### 41.4 双语与内容一致性

- `en` 与 `zh` 页面必须同步提供输入、结果、方案、故障预览、导出按钮和反馈信息；不得出现一侧缺少功能或中英混排。
- 公式、单位和计算逻辑保持一致，仅翻译界面、说明、建议、FAQ 与 SEO 内容。
- 新增字段必须同时加入两种语言、可访问性标签和导出列名。

### 41.5 发布前验收门槛

每个页面完成后必须逐项留存验证证据：

1. 计算引擎测试覆盖正常、边界和故障场景。
2. 中英文桌面与移动端页面测试通过，确认无控制台错误、布局溢出和功能缺失。
3. 方案比较、故障预览、打印/报告导出、BOM 导出逐项实际点击验证。
4. 抽查导出文件，确认网站地址、时间戳、输入值和结果值齐全且可追溯。
5. 独立审计员依据本节标准复核实际页面和测试证据；任一项未通过，不得合并或推送生产。
