"use strict";const CACHE = "nel-telecom-ac-dc-breaker-sizing-locale-v1.9.9-04-p0-p1-correctness-consent-offline-ca275433f826-ba16ae3c8ac7";
const CORE = [
  "./",
  "./index.html",
  "./zh/",
  ...
  "./css/style.css?v=bb05c0f6810b",
  "./js/engine.js",
  "./js/app.js?v=80543006d4f1",
  "./js/pwa.js",
...
];self.addEventListener("install",e=>{e.waitUntil(caches.open(CACHE).then(c=>c.addAll(CORE)));self.skipWaiting()});self.addEventListener("activate",e=>{e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith("nel-telecom-ac-dc-breaker-sizing-")&&k!==CACHE).map(k=>caches.delete(k)))));self.clients.claim()});self.addEventListener("fetch",e=>{if(e.request.method!=="GET")return;e.respondWith(caches.match(e.request,{ignoreSearch:true}).then(c=>c||fetch(e.request)))});