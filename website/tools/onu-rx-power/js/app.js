...
const T={
 zh:{
  ...
  invalid:"请检查输入：数量必须为非负整数，衰减系数必须大于0，分光损耗必须来自支持列表，疑似无光门限必须低于灵敏度，灵敏度必须低于过载门限。",
  invalidBadge:"输入无效",
 ...
 en:{
  ...
  invalid:"Check inputs. Counts must be non-negative integers; attenuation must be positive; splitter loss must be supported; no-signal must be below sensitivity, and sensitivity below overload.",
  invalidBadge:"Invalid input",
...
function invalidate(){
 last=null;
 ["measuredResult","sensitivityMargin","overloadMargin","receiverWindow","expectedRx","deviation","inferredExtra","modeledLoss","fiberLoss","spliceTotal","connectorTotal","splitterLoss"].forEach(id=>$(id).textContent="—");
 $("splitRatio").textContent="—";$("gaugePower").textContent="—";$("gaugeMarker").style.left="0";
 $("healthBadge").textContent=T[LANG].invalidBadge;$("summaryCard").className="summary-card";$("diagnosis").className="diagnosis";
 $("diagnosisText").textContent=T[LANG].invalid;$("causeList").innerHTML="";$("validation").textContent=T[LANG].invalid;
...
function setMode(mode){
 ...
 $("modelResults").classList.toggle("hidden",mode==="measured");
...
function calculate(){
 ...
 if(!result.ok){invalidate();return}$("validation").textContent="";
 ...
 if(v.mode==="model"){
  ...
  ["expectedRx","deviation","inferredExtra","modeledLoss","fiberLoss","spliceTotal","connectorTotal","splitterLoss","splitRatio"].forEach(id=>$(id).textContent="—");
 }
...
function report(){
 ...
 const lines=["NetEngineerLab - ONU RX Power Diagnosis",`${LANG==="zh"?"工程":"Project"}: ${r.project}`,`${LANG==="zh"?"实测接收功率":"Measured RX"}: ${r.measured.toFixed(2)} dBm`,`${LANG==="zh"?"灵敏度余量":"Sensitivity margin"}: ${r.sensitivityMargin.toFixed(2)} dB`,`${LANG==="zh"?"过载余量":"Overload margin"}: ${r.overloadMargin.toFixed(2)} dB`];
 if(r.mode==="model")lines.push(`${LANG==="zh"?"理论接收功率":"Expected RX"}: ${r.expectedRx.toFixed(2)} dBm`,`${LANG==="zh"?"实测偏差":"Measured deviation"}: ${r.deviation.toFixed(2)} dB`,`${LANG==="zh"?"推算额外损耗":"Inferred excess loss"}: ${r.inferredExtra.toFixed(2)} dB`);
 lines.push(`${LANG==="zh"?"状态":"Status"}: ${T[LANG][r.status]}`);return lines.join("\n")
}
...
function save(){if(!last)calculate();if(!last)return;const h=getHistory();h.unshift(last);localStorage.setItem("onuRxDiagnosisHistory",JSON.stringify(h.slice(0,12)));renderHistory();temp($("saveBtn"),T[LANG].saved)}
...
function csv(){
 if(!last)calculate();if(!last)return;const r=last,rows=[["Project",r.project],["Measured RX dBm",r.measured.toFixed(2)],["Receiver sensitivity dBm",r.sens],["Overload threshold dBm",r.over],["Sensitivity margin dB",r.sensitivityMargin.toFixed(2)],["Overload margin dB",r.overloadMargin.toFixed(2)]];
 if(r.mode==="model")rows.push(["Expected RX dBm",r.expectedRx.toFixed(2)],["Deviation dB",r.deviation.toFixed(2)],["Inferred excess loss dB",r.inferredExtra.toFixed(2)]);rows.push(["Status",T[LANG][r.status]]);
...
["projectName","measuredRx","rxSensitivity","rxOverload","noSignalThreshold","warningMargin","txPower","distance","attenuation","spliceCount","spliceLoss","connectorCount","connectorLoss","splitter1","splitter2","otherLoss"].forEach(id=>$(id).addEventListener("input",()=>{invalidate();clearTimeout(window.__onu);window.__onu=setTimeout(calculate,160)}));
