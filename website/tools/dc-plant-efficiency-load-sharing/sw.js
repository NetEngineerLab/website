"use strict";const CACHE = "nel-dc-plant-efficiency-load-sharing-locale-v1.9.9-04-p0-p1-correctness-consent-offline-28ce5f771f70-43039aee43cb";
const CORE = [
  "./",
  "./index.html",
  "./zh/",
  ...
  "./css/style.css?v=f9455ab9cb6f",
  "./js/engine.js",
  "./js/app.js?v=07e941f4d9a4",
  "./js/pwa.js",
...
];self.addEventListener("install",e=>{e.waitUntil(caches.open(CACHE).then(c=>c.addAll(CORE)));self.skipWaiting()});self.addEventListener("activate",e=>{e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith("nel-dc-plant-efficiency-load-sharing-")&&k!==CACHE).map(k=>caches.delete(k)))));self.clients.claim()});self.addEventListener("fetch",e=>{if(e.request.method!=="GET")return;e.respondWith(caches.match(e.request,{ignoreSearch:true}).then(c=>c||fetch(e.request).catch(()=>caches.match("./offline.html"))))});