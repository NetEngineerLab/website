"use strict";const CACHE = "nel-generator-fuel-runtime-calculator-locale-v1.9.9-04-p0-p1-correctness-consent-offline-28ce5f771f70-052b819e6dbd";
const CORE = [
  "./",
  "./index.html",
  "./zh/",
  ...
  "./css/style.css?v=d4cb772eacc2",
  "./js/engine.js",
  "./js/app.js?v=4d21c42736e6",
  "./js/pwa.js",
...
];self.addEventListener("install",e=>{e.waitUntil(caches.open(CACHE).then(c=>c.addAll(CORE)));self.skipWaiting()});self.addEventListener("activate",e=>{e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith("nel-generator-fuel-runtime-calculator-")&&k!==CACHE).map(k=>caches.delete(k)))));self.clients.claim()});self.addEventListener("fetch",e=>{if(e.request.method!=="GET")return;e.respondWith(caches.match(e.request,{ignoreSearch:true}).then(c=>c||fetch(e.request)))});