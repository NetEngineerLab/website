"use strict";const CACHE = "nel-telecom-solar-battery-sizing-calculator-locale-v1.9.9-04-p0-p1-correctness-consent-offline-0acfeb4bb3ce-064d894d7d4d";
const CORE = [
  "./",
  "./index.html",
  "./zh/",
  ...
  "./css/style.css?v=7b3171c4c126",
  "./js/engine.js",
  "./js/app.js?v=b6f146866077",
  "./js/pwa.js",
...
];self.addEventListener("install",e=>{e.waitUntil(caches.open(CACHE).then(c=>c.addAll(CORE)));self.skipWaiting()});self.addEventListener("activate",e=>{e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith("nel-telecom-solar-battery-sizing-calculator-")&&k!==CACHE).map(k=>caches.delete(k)))));self.clients.claim()});self.addEventListener("fetch",e=>{if(e.request.method!=="GET")return;e.respondWith(caches.match(e.request,{ignoreSearch:true}).then(c=>c||fetch(e.request)))});