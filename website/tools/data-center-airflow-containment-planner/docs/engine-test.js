...
assert(r.scenarios.peak.designKW>r.scenarios.baseline.designKW);assert(r.scenarios.conservative.designKW>r.scenarios.peak.designKW);assert(r.beforeAfter.delta.designKW>0);assert(typeof E.toJSON(E.buildReport({rackCount:12})).includes('airflow-containment-report/2.0.0'));assert(E.toCSV(r).includes('baseline'));
