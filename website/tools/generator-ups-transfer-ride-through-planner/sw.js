"use strict";const CACHE = "nel-generator-ups-transfer-ride-through-planner-locale-v1.9.9-04-p0-p1-correctness-consent-offline-a5ad54e03080-0de112dd326e";
const CORE = [
  "./",
  "./index.html",
  "./zh/",
...
];self.addEventListener("install",e=>{e.waitUntil(caches.open(CACHE).then(c=>c.addAll(CORE)));self.skipWaiting()});self.addEventListener("activate",e=>{e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith("nel-generator-ups-transfer-ride-through-planner-")&&k!==CACHE).map(k=>caches.delete(k)))));self.clients.claim()});self.addEventListener("fetch",e=>{if(e.request.method!=="GET")return;e.respondWith(caches.match(e.request,{ignoreSearch:true}).then(c=>c||fetch(e.request)))});