"use strict";const CACHE = "nel-olt-dual-uplink-transport-mse-planner-locale-v1.9.9-04-p0-p1-correctness-consent-offline-28ce5f771f70-1caaf13c3f60";
const CORE = [
  "./",
  "./index.html",
  "./zh/",
  ...
  "./css/style.css?v=588fbd7e35e7",
  "./js/engine.js",
  "./js/app.js?v=e8627f68c359",
  "./js/pwa.js",
...
];self.addEventListener("install",e=>{e.waitUntil(caches.open(CACHE).then(c=>c.addAll(CORE)));self.skipWaiting()});self.addEventListener("activate",e=>{e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith("nel-olt-dual-uplink-transport-mse-planner-")&&k!==CACHE).map(k=>caches.delete(k)))));self.clients.claim()});self.addEventListener("fetch",e=>{if(e.request.method!=="GET")return;e.respondWith(caches.match(e.request,{ignoreSearch:true}).then(c=>c||fetch(e.request)))});